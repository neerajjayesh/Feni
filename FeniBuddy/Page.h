...
<style>
:root{color-scheme:dark;--bg:#101c23;--panel:#1a2b34;--ink:#e9f2ed;--muted:#a6b9bf;--accent:#22d3ee}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:16px/1.5 system-ui,sans-serif}main{max-width:980px;margin:auto;padding:32px 20px 64px}header{margin-bottom:26px}h1{font-size:42px;letter-spacing:-2px;margin:0}h2{font-size:21px;margin:0 0 12px}p{margin:8px 0 18px;color:var(--muted)}.tag{color:var(--accent);letter-spacing:2px;font-size:12px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}section{background:var(--panel);padding:24px;border:1px solid #33454c;border-radius:3px}section.wide{grid-column:1/-1}label{display:block;margin:13px 0 5px;font-size:14px}input,select,button{font:inherit;border:1px solid #62777e;border-radius:3px;padding:10px 12px}input,select{width:100%;background:#12232b;color:var(--ink)}button{background:var(--accent);color:#13261c;cursor:pointer;font-weight:600;margin:14px 8px 0 0}button.secondary{background:transparent;color:var(--ink)}button:disabled{opacity:.5;cursor:wait}button:focus-visible,input:focus-visible,select:focus-visible{outline:3px solid #81bcff;outline-offset:2px}small{display:block;color:var(--muted);margin-top:5px}.live{display:flex;align-items:center;gap:24px}.preview{width:240px;max-width:100%;aspect-ratio:5/4;image-rendering:pixelated;background:black;border:5px solid #0b1419;border-radius:12px}.status{color:var(--accent)}#notice{position:sticky;top:8px;background:#dff8e7;color:#16301e;padding:12px 16px;border-radius:3px;z-index:2;white-space:pre-line}#notice:empty{display:none}.preset{display:grid;grid-template-columns:85px 1fr;gap:10px;margin:8px 0}details{margin-top:16px}summary{cursor:pointer;color:var(--accent)}a{color:var(--accent)}.steps{padding-left:22px;color:var(--muted)}code{color:var(--ink)}#events{padding-left:22px;color:var(--muted)}@media(max-width:680px){.grid{grid-template-columns:1fr}.live{flex-direction:column;align-items:start}h1{font-size:36px}section{padding:19px}}
</style></head><body><main>
<header><div class="tag">FENI / DESK DISPLAY</div><h1>Feni</h1><p id="connection">Connecting to your buddy...</p></header>
<div id="notice" role="status" aria-live="polite"></div>
<div class="grid">
<section class="wide"><div class="live"><img id="screen" class="preview" alt="Live copy of Feni's display"><div><h2>Your desk buddy</h2><p>Tap to move. Hold to choose.<br>Double tap to go back from any screen.<br>After 1 minute idle, Feni returns to the face.</p><button data-action="tap">Tap</button><button data-action="hold">Hold</button><button data-action="back" class="secondary">Back</button><small id="timerState"></small></div></div></section>
...
for(const [id,path] of [['wifiForm','/wifi'],['modeForm','/settings'],['calendarForm','/settings'],['wledForm','/settings'],['timerForm','/timer'],['themeForm','/theme']]){
...
async function refresh(){
 ...
 document.documentElement.style.setProperty('--accent',['#22d3ee','#ff9b42','#4ade80','#b388ff'][state.theme||0]);
 ...
 $('events').replaceChildren();for(const event of state.events){const row=document.createElement('li');row.textContent=event.title+' | '+(event.allDay?'all day':new Date(event.start*1000).toLocaleString());if(event.color565){const c=event.color565;row.style.borderLeft='5px solid rgb('+[(c>>11)*255/31,((c>>5)&63)*255/63,(c&31)*255/31].join(',')+')';row.style.paddingLeft='8px';} $('events').append(row);}
 $('wledState').textContent=state.wledMessage;
 if(!initialized){$('theme').value=state.theme||0;$('ssid').value=state.ssid;$('mode').value=state.mode;$('timezone').value=state.timezone;$('wledIp').value=state.wledIp;state.presets.forEach((p,i)=>{$('p'+i+'id').value=p.id;$('p'+i+'name').value=p.name;});initialized=true;}
